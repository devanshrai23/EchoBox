...
export default function SendMessage() {
  ...
  const [suggestedString, setSuggestedString] = useState(initialMessageString);
  ...
  const onSubmit = async (data: z.infer<typeof messageSchema>) => {
    ...
    try {
      if (!username) return;
      const response = await sendMessage(username as string, data.content);
      ...
      // @ts-ignore
      toast.create({ title: 'Error', description: 'Failed to send message', type: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="space-y-4 my-12">
        ...
        <Card>
          ...
          <CardContent className="flex flex-col space-y-4">
            {parseStringMessages(suggestedString).map((message, index) => (
              <Button
                key={index}
                variant="outline"
